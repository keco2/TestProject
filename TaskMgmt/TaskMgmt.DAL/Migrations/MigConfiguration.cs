namespace TaskMgmt.DAL.Migrations
{
    ...
    internal sealed class MigConfiguration : DbMigrationsConfiguration<TaskMgmt.DAL.TaskMgmtDbContext>
    {
        ...
        protected override void Seed(TaskMgmt.DAL.TaskMgmtDbContext context)
        {
            ...
            var task1 = new TaskEntity() { ID = new Guid("00000000-0000-0000-0000-000000000001"), Name = "Task 1 created by Migration-Seed" };
            var task2 = new TaskEntity() { ID = new Guid("00000000-0000-0000-0000-000000000002"), Name = "Task 2 created by Migration-Seed" };
            var task3 = new TaskEntity() { ID = new Guid("00000000-0000-0000-0000-000000000003"), Name = "Task 3 created by Migration-Seed" };

            taskDbSet.AddOrUpdate(task1);
            taskDbSet.AddOrUpdate(task2);
            taskDbSet.AddOrUpdate(task3);
            ...
            DbSet<TaskMaterialUsageEntity> usageDbSet = context.Set<TaskMaterialUsageEntity>();

            var usage1 = new TaskMaterialUsageEntity() { TaskID = task1.ID, Task = task1, MaterialID = material1.ID, Material = material1, Amount = 1, UnitOfMeasurement = "kg" };
            var usage2 = new TaskMaterialUsageEntity() { TaskID = task1.ID, Task = task1, MaterialID = material3.ID, Material = material3, Amount = 2, UnitOfMeasurement = "pcs" };
            var usage3 = new TaskMaterialUsageEntity() { TaskID = task2.ID, Task = task2, MaterialID = material1.ID, Material = material1, Amount = 3, UnitOfMeasurement = "g" };

            //usageDbSet.AddOrUpdate(usage1);
            //usageDbSet.AddOrUpdate(usage2);
            //usageDbSet.AddOrUpdate(usage3);
